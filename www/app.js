(function () {
  ...
  async function saveConfig() {
    ...
    setStatus('Saved');
    // Refresh to get the canonical normalized state.
    applyToUI(body);
  }
  ...
  $$('.tab').forEach((t) => t.addEventListener('click', () => {
    ...
    if (t.dataset.tab === 'log') fetchLog();
  }));

  // Add buttons for the scoped tables
  document.body.addEventListener('click', (e) => {
    ...
    if (add === 'ip-allow') { (cfg.ip_allowlist ||= []).push({ value: '' }); renderScopedTable('ip-allow-table', cfg.ip_allowlist); }
      ...
      tr.remove();
      // Re-index visible rows so subsequent saves stay consistent.
  ...
  $('#log-refresh').addEventListener('click', fetchLog);
